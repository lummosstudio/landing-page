      ...
      <div className="mx-auto flex max-w-[1440px] items-center gap-6 sm:gap-8 px-4 sm:px-6 lg:gap-4 lg:px-8">
        <Reveal direction="right" delay={100} className="hidden lg:block shrink-0">
          <img
            ...
            height={700}
            loading="lazy"
            className="size-[180px] shrink-0 object-contain xl:size-[220px] transition-transform duration-500 hover:scale-105"
          />
        </Reveal>
        ...
        <ol className="grid w-full gap-y-8 sm:gap-y-10 grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 lg:gap-x-2">
          {STEPS.map(({ icon: Icon, number, title, description, accent }, i) => (
            ...
            >
              <Reveal direction="up" delay={i * 80} className="w-full flex flex-col items-center">
                {i < STEPS.length - 1 && (
                  <span
                    aria-hidden="true"
                ...
                <span
                  className={`relative inline-flex size-[52px] sm:size-[56px] items-center justify-center rounded-full transition-transform duration-300 hover:scale-110 ${
                    accent ? "bg-brand-gradient shadow-brand-btn" : "bg-navy shadow-navy-btn"
                  }`}
                >
                  <Icon
